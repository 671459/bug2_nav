...
class GoToPointClass(Node):
    ...
    def service_callback(self, request, response):
        #Set self.active based on the request's 'activate' field
        self.active = request.activate

        #Set the desired position from the request message
        self.desired_position.x = request.target_position.x
        self.desired_position.y = request.target_position.y
        self.desired_position.z = request.target_position.z

        #Reset state to start the process from the beginning
        if self.active:
            self.change_state(0)
            self.get_logger().info('Service activated, moving to target position: [' 
                                   + str(self.desired_position.x) + ', ' 
                                   + str(self.desired_position.y) + ', ' 
                                   + str(self.desired_position.z) + ']')
        else:
            self.done()
            self.get_logger().info('Service deactivated')
