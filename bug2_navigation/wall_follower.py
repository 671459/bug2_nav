...
class WallFollowerClass(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(
            SetBool, "wall_follower_service", self.service_callback
        )
    ...
    def service_callback(self, request, response):
        #Set self.active based on the request's 'data' field
        self.active = request.data

        #Log the activation or deactivation
        if self.active:
            self.get_logger().info('Wall follower activated')
        else:
            self.get_logger().info('Wall follower deactivated')

        #Prepare response
        response.success = True
        response.message = 'Wall follower ' + ('activated' if self.active else 'deactivated')
        return response
